...
from vint_train.models.nomad.CNN import SensorModel
...
def evaluate_nomad(
    ...
    with tqdm.tqdm(
        ...
        for i, data in enumerate(tepoch):
            ...
            # pre_pt = torch.stack(pre_pt,dim=0)
            # def save_tensors(dpos, i, file_path):
            #     data = {f"traj_{i}_dpos": dpos}
            #     torch.save(data, file_path)
            #     print(f"Tensors saved to {file_path}")
            # save_tensors(pre_pt, i, f"pt/traj_{i}_data.pt")

            losses = _compute_losses_nomad(predict_dpositions, ground_truth, theta0, p0, pT)
...
def main(config):
    ...
    class state_extractor(nn.Module):
        ...
        #     vector_low_expanded = self.low_dim_to_high(vector_low)  # (B, 128)
        #     fused = torch.cat((vector_high, vector_low_expanded), dim=1)  # (B, 256)
...
if __name__ == "__main__":
    ...
    config_route = "/home/yuyu/diffusion_model/state_estimation_kitti10/train/config/state_vio_fold_1_sim.yaml"
    with open(config_route, "r") as f:
        user_config = yaml.safe_load(f)
    config = user_config
    config['load_run'] = '/home/yuyu/diffusion_model/state_estimation_kitti10/train/logs/state_vio_fold1/s1simple'
    main(config)