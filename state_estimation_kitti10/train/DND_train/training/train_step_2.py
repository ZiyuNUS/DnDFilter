...
def _compute_losses_dnd_train(
    ...
    model_output_dict = model_output_train(
        ema_model,
        noise_scheduler,
        batch_obs_images,
        model_info,
...
def train_dnd(
    ...
    with tqdm.tqdm(dataloader, desc="Train Batch", leave=False) as tepoch:
        for i, data in enumerate(tepoch):
            ...
            mask = (torch.rand(99) < 0.5).to(device)
            model_info[mask] = 0
            ...
            if i % print_log_freq == 0:
                losses = _compute_losses_dnd_train(
