...
def _compute_losses_nomad(
    ...
    # np.save('gc_actions_x'+ str(i) +'.npy', gc_actions[:, 0])
    # np.save('gc_actions_y'+ str(i) +'.npy', gc_actions[:, 1])
    # np.save('ground_truth_x'+ str(i) +'.npy', ground_truth[:, 0])
    # np.save('ground_truth_y'+ str(i) +'.npy', ground_truth[:, 1])
...
def main(config):
    ...
    if "load_run" in config:
        ...
        latest_path = os.path.join(load_project_folder, "ema_199.pth")
...
if __name__ == "__main__":
    ...
    config_route = "config/state_128_100_3.yaml"
    with open(config_route, "r") as f:
        user_config = yaml.safe_load(f)
    config = user_config
    config['load_run'] = '/home/yuyu/diffusion_model/state-estimation_image/train/logs/state/128_100_3'
    main(config)
