...
class KalmanFilter(nn.Module):
    ...
    def forward(self, z_list, L_hat_list, pos0s, vel0s, simplified_cov_update):
        ...
        # Initial belief state (center frame and zero velocity) -> no requires grad because needed only once (N,2)
        ...
        # Initial state covariance initialised on random variances -> no requires grad because needed only once (N,4,4)
    ...
    def kalman_gain(self, covar_predicted, R, Cz):
        ...
        epsilon = 1e-4
        matrix_to_invert = Cz @ covar_predicted @ Cz.transpose(-2, -1) + R + epsilon * torch.eye(R.size(-1), device=R.device)
        return covar_predicted @ Cz.transpose(-2, -1) @ matrix_to_invert.inverse()
    ...
    def kf_update(self, mean_input, covar_input, z, L_hat, simplified_cov_update, t):
