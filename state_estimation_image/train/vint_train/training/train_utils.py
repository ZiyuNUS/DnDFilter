...
#             # for j in range(90):
#             #     for i in range(5):
#             #         image = obs_images[i][0].permute(1, 2, 0)  # 将 (3, 128, 128) 转为 (128, 128, 3)
...
def train_nomad(
    ...
    with tqdm.tqdm(dataloader, desc="Train Batch", leave=False) as tepoch:
        ...
        if use_wandb and wandb_log_freq != 0:
            log_data = {
                'pos_loss (train)': total_train_loss / (i + 1),
            }
...
def evaluate_nomad(
    ...
    with tqdm.tqdm(
        ...
        if use_wandb and wandb_log_freq != 0:
            log_data = {
                'pos_loss (test)': total_test_loss / (i + 1),
            }
            wandb.log(log_data, commit=True)
