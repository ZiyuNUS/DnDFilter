...
def _compute_losses_nomad(
    ...
    gc_actions = predict_positions[:,1,:].cpu().detach().numpy()
    ground_truth = ground_truth[:,1,:].cpu().detach().numpy()

    time_steps = range(gc_actions.shape[0])  # 生成 0 到 95 的时间步列表

    plt.figure(figsize=(20, 20))
    plt.subplot(3, 1, 1)  # 第一张子图
    plt.plot(time_steps, gc_actions[:, 0], label='GC Actions (x)')
    plt.plot(time_steps, ground_truth[:, 0], label='Ground Truth (x)')
    # plt.plot(time_steps, indicator[:, 0], label='Ground Truth (x)')
    plt.grid(True)

    plt.subplot(3, 1, 2)  # 第二张子图
    plt.plot(time_steps, gc_actions[:, 1], label='GC Actions (y)')
    plt.plot(time_steps, ground_truth[:, 1], label='Ground Truth (y)')
    # plt.plot(time_steps, indicator[:, 1], label='Ground Truth (x)')
    plt.grid(True)

    plt.subplot(3, 1, 3)
    # plt.plot(time_steps, block, label='Ground Truth (x)')
    plt.plot(time_steps, [10] * len(time_steps), label='Ground Truth (x)')

    plt.tight_layout()
    global i
    plt.savefig('images/output' + str(i) + '.png')
    i = i + 1
    plt.close()  # 关闭当前图表，防止占用内存

    if i == 8:
        np.save('gc_actions_x_g' + str(i) + '.npy', gc_actions[:, 0])
        np.save('gc_actions_y_g' + str(i) + '.npy', gc_actions[:, 1])
        np.save('ground_truth_x_g' + str(i) + '.npy', ground_truth[:, 0])
        np.save('ground_truth_y_g' + str(i) + '.npy', ground_truth[:, 1])
...
def main(config):
    ...
    if "load_run" in config:
        ...
        latest_path = os.path.join(load_project_folder, "ema_330.pth")
...
if __name__ == "__main__":
    ...
    config_route = "config/step_10.yaml"
    with open(config_route, "r") as f:
        user_config = yaml.safe_load(f)
    config = user_config
    config['load_run'] = '/home/yuyu/diffusion_model/state-estimation_image/train/logs/state/diff_10_s1'
    main(config)
