...
def _compute_losses_nomad(
    ...
    ground_truth = unnormalize_data(ground_truth, ACTION_STATS)
    ...
    plt.plot(time_steps, [8] * len(time_steps), label='Ground Truth (x)')
    ...
    delta = gc_actions - ground_truth
    delta_X = delta[:, 0]
    delta_Y = delta[:, 1]
    mask_x = np.abs(delta_X) > 1.5
    mask_y = np.abs(delta_Y) > 1.5
    block_values = block[mask_x|mask_y]
    indices = np.where(mask_x | mask_y)
